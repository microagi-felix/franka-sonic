...
def main() -> None:
    ...
    for i in range(args.procs):
        ...
        # Isaac's scripts end with os._exit() (the headless close() hang), which skips the
        # stdout flush — round 1's worker logs therefore contained NONE of the datagen's
        # `k/n (x%) successful demos generated by mimic` progress and the keep rate came out
        # `nan`. Unbuffered stdout is the whole fix.
    ...
    # -1 (not nan) when no worker log carried a progress line: downstream parses a number.
    rate = (100.0 * succ / trials) if trials else -1.0
