...
ds=$(find "$RUNS" "$HOME/datasets/franka-sonic" -maxdepth 7 -type f -name modality.json \
       -path '*/meta/*' 2>/dev/null | grep -iE 'sonic|token' | head -1)
if [ -n "$ds" ]; then
...
# 2 -------------------------------------------------------------- checkpoint-2000
ckpt=$(find "$LANE_B" -maxdepth 6 -type d -name 'checkpoint-2000' 2>/dev/null | head -1)
if [ -n "$ckpt" ]; then
...
# 3 -------------------------------------------------------------- policy eval >= 20
csv=$(find_eval_csv "$LANE_B" "" "oracle")
if [ -n "$csv" ]; then
  rows=$(csv_rows "$csv")
...
# 4 -------------------------------------------------------------- B-oracle
ocsv=$(find_eval_csv "$LANE_B" "oracle" "")
if [ -n "$ocsv" ]; then
  orows=$(csv_rows "$ocsv")
