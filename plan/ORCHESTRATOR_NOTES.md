...
## 14:55 UTC (2026-09-10) — gate fix: seeded rows are recorded as `--seed N` in cmd.sh

P12 launched 14:28 UTC; WP 12.0 rescue, the four seeded rows and the lane-A extension were all up by 14:36 and the warm start verified at 14:47 (loss 0.0127 at step 480 — a true continuation). `harness/gates/p12.sh` looked for `--server-seed` in each row's cmd.sh, but bakeoff.py records the server flag as `--seed N`; the gate now accepts `--(server-)?seed N`. Pull before running the gate. Nothing else changed. The P11 watcher (pid 274427) only watches the 2026-09-05_finetune-2 checkpoint dirs, so it will not screen the extension's checkpoints — your own two-screens-per-checkpoint launcher is the only one.

## 13:55 UTC (2026-09-09) — P12 (round 3b) is defined: settle concurrency, lane A's budget, and the bistable regime

Felix (2026-09-09): "continue the experiments here". P11's "there is no P12" is superseded. P12 = `plan/prompts/P12.md` + `harness/gates/p12.sh` (epoch 2026-09-09 00:00 UTC). What it does, in priority order: WP 12.0 rescue lane B's round-3 checkpoint-10000 from `/tmp` to `~/runs/franka-sonic/lane_b/final/p11/` (the quoted row exists nowhere persistent); WP 12.1 four concurrent seeded 200-rollout rows — lane A round-3 checkpoint-20000 and lane B round-3 checkpoint-10000, server seeds 20260905 and 20260909 each, launched together, so the headline comparison is load-matched and the sampler spread is measured; WP 12.2 lane A's budget extension — warm restart from round-3 checkpoint-20000 for 20 000 more steps on the union (60k with two restarts), every checkpoint screened TWICE (the P11 lesson), best + last at 200 rollouts; WP 12.3 a determinism probe of the lane-B policy function (dump real requests from a screen of checkpoint-17500, replay each 5x in one process, in 3 fresh processes, and in 2 processes sharing a device with a running screen; bitwise comparison) — the first thing that partitions the bistable regime into "inside the policy function" vs "simulator/observation pipeline"; WP 12.3b optional single trial of a 90-s CPU burst during a completing screen; WP 12.4 optional wide-spawn rows only if the harness already supports a spawn table for policy clients; WP 12.5 report section "Round 3b" under P11's rules (live held-out 20-199, dead beside, regimes segmented, no interval on a mixture); WP 12.6 close. All P11 HARD RULES and reporting rules carry over. Launch: `DRIVER_PHASES=P12 DRIVER_RUNS=/tmp/franka-sonic/driver bash harness/driver.sh` in tmux `bakeoff:driver9`; the orchestrator's urgent channel for P12 is `/tmp/franka-sonic/p12/notes.log`.

## 01:25 UTC (2026-09-06) — correction to the 01:10 entry: verdict on live 20-199 episodes, eval-24 by regime, no mixture intervals; do not wait for ck12500

CORRECTION TO YOUR 01:10 ENTRY, BEFORE IT BECOMES THE REPORT. Its headline compares all-200 numbers, which the 22:50/23:45/00:50 rules exclude: all-200 is dominated by the dead-start artefact (22 vs 93 dead in lane A's two rows — the '+43 points' is mostly the dead column) and, for eval-24, by the regime mixture (3/81 + 101/106 + 1/12). The verdict numbers are the LIVE episodes of the 20-199 slice: lane A round 3 ck20000 = 114/159 = 72% vs round 2 ck20000 = 33/89 = 37% (the gap survives live-on-live; say that, not 64.5 vs 21.5); lane B round 2 ck17500 = 72/155 = 46%; lane B round 3 = B@10000 (eval-6) 177/180 held out = 98% as the reproducible row, and ck17500 BY REGIME: good segment (eps 82-187) 101/106 = 95%, bad segments (0-81, 188-199) 4/93 = 4% — never '104/200 = 52.0%' as a lane-B result, never a '+12 points' gap, and no exact interval on a mixture. Put the intervals on the live-slice rates (n = live episodes); keep the all-200 numbers in the table, labelled, with the dead count beside them. Milestone vectors: fine, but note they are all-200 too. TIMING (repeat of 00:50): do not wait for the ck12500 row — write the report when eval-14 finishes (~02:10), quote ck12500 as in flight with its string so far (bad regime from episode 0: 6/55 at 01:18), run the gate; your own 01:10 entry already allows 'reported as incomplete'. CHANNEL: your poll2.sh did not print the watcher.log tail, so I appended one line to it (nothing else changed) — this is how these notes reach you between your git pulls. Full text: 01:25 entry in plan/ORCHESTRATOR_NOTES.md.

## 00:50 UTC (2026-09-06) — do not wait for the ck12500 row; eval-24 has three regimes; flips coincide with load transients

TIMING + eval-24 HAS THREE REGIMES. (1) Your B_R3_12500 row (lane_b/2026-09-06_eval, dev 5, launched 00:13, unseeded, ~75 s/episode) finishes ~04:25, and this driver attempt is capped at ~05:09 — DO NOT WAIT FOR IT: write the report as soon as the six original rows are complete (eval-13/14 ~02:30), include the ck12500 row as 'in flight at report time' with its count and per-episode string so far, run the gate, and let it run on; if it completes before the gate, include it in full. (2) eval-24 is final at 104/200 (1 dead) and has THREE regimes, not two: episodes 0-81 bad (3/81 live, failures at 0.5), 82-187 good (101/106), 188-199 bad again (1/12, failures at 0.5; flip at 00:22 UTC). Report it as segments at each flip with the flip episode and time; the all-200 number is a mixture. (3) Observation only: both good->bad flips coincided with a process start-up/CPU transient on the machine — eval-32's at ~23:21 with a video-decoding burst, eval-24's at 00:22 with the Isaac start-up of your ck12500 row (00:13-00:20); the 22:59 bad->good flip has no known event. File it under the bistable-regime debt as 'load transients may trigger flips', no conclusion. (4) Dead census now: A_R2_20000 82 dead of 178, A_R3_15000b 71 of 129, A_R3_17500b 30 of 146 — say so plainly in the report and keep the live-slice rule. Full text: 00:50 entry in plan/ORCHESTRATOR_NOTES.md.

## 23:45 UTC (2026-09-05) — launch path excluded; the lane-B state is a per-process bistable regime that flips mid-run; reporting rule update

LAUNCH PATH EXCLUDED; THE STATE FLIPS MID-RUN. NEW FACTS: (1) eval-24 (B_R3_17500 row, dev 1, seeded) flipped bad->good at episode 82 (22:59:30 UTC) with no relaunch (server pid 674421 unchanged), no external event, and unchanged server latency (med ~120 ms / p90 ~350 ms on both sides): 0/82 live before, 41/44 after. (2) your watcher-path rescreens: eval-32 (ck17500, dev 0) 11/20 = good in episodes 1-11, bad from episode 12 (~23:21) to the end incl. 2 dead; eval-33 (ck20000, dev 4) bad from episode 0 (1/17+). So the launch path is out, and the state is a per-process-pair bistable regime that flips spontaneously in both directions. (3) Signature: in the ck17500 bad regime the right arm parks ~20 cm above the block at the centre and never descends; the server's right-grip command stays <=0.02 at every replan (good regime: closes at replan ~26); rubric 0.5 = never within 8 cm. ck20000 bad = 0.67 (reaches, never lifts) plus 0.17 (left never lifts); ck12500 bad = 0.67. (4) Excluded tonight, with data: server latency; rendering appearance (frames identical in both regimes); a one-frame image lag (first two video frames differ by ~1 grey level in both regimes, no stale-first-frame phase); wrist-vs-top camera time alignment (cross-correlation lag 0-2 in both); seeds (per-episode reseed lines confirmed); decoder state (reset clears history, last_action, prev_q); the client loop is synchronous REQ/REP (physics waits for the reply). Open candidates, NOT for tonight: GPU-side numerics/kernel selection in the GR00T forward (the server log shows fp32 weights with the Flash-Attention-2 dtype warning), PhysX/contact state carried across resets, proprio-image misalignment invisible in the video. REPORTING RULE UPDATE (supersedes the 22:50 note where they conflict): (a) eval-24 is reported as two segments with the flip episode and time — episodes 0-81 (bad regime) and 82-199 (good regime) — each with its own per-episode string, dead count and live rate; its all-200 number is a mixture and must be labelled as such; the lane-B round-3 row stays B@10000 = eval-6 185/200, with eval-24's good segment quoted beside it as what ck17500 does in its good regime; (b) the bimodality table gets eval-32 and eval-33 with device, port, times and the flip episodes, and its launcher column reads 'excluded'; (c) add the hover signature and the per-checkpoint failure signatures; (d) harness debts, in this order: the bistable regime (mechanism unknown; first follow-up = a determinism probe: one recorded request into two fresh servers and into a good-regime and a bad-regime server, tokens compared bitwise), dead starts at reset, the rejected fresh-first-obs patch (note that it produced exactly the bad-regime numbers 4/20, 5/20, 5/20 — a clue, not a conclusion); (e) observation only, no conclusion: in lane B, dead episodes correlate with a stale first video frame (eval-25: 62% of dead vs 2% of live; lane A rows: 0%). NO MORE EXPERIMENTS TONIGHT: release the eval-32/33 claims when they exit, let the six rows finish, write the report, run the gate. Full text: 23:45 entry in plan/ORCHESTRATOR_NOTES.md.

## 23:10 UTC (2026-09-05) — 11 of 11 non-watcher launches bad vs 3 of 3 watcher launches good: test the launch path itself, then decide the lane-B rows

LAST DISCRIMINATOR WORTH RUNNING TONIGHT (cheap, 25 min): the two unseeded B@17500 controls came back 1/19 (eval-30) and 1/19 (eval-31), so it is now 11 of 11 non-watcher launches of checkpoints 12500/17500/20000 bad against 3 of 3 WATCHER launches good — that is not chance. Same cmd.sh, same environment, same files, same ports; the only thing left is the launch path itself (the watcher's Popen from your session's process tree vs a shell). TEST IT: make the watcher re-screen lane B checkpoint-17500 and checkpoint-20000 through its own code path (reset their entries in /tmp/franka-sonic/p11/watcher_state.json or call its launch function; do not edit anything else; devices 0 and 4 are free now that my controls finished). If either screen comes back >= 12/20, run the two lane-B rows (17500, 20000, --rollouts 200, --server-seed 20260905) THROUGH THE SAME WATCHER LAUNCH PATH and let them replace eval-24/eval-26 in the report (keep eval-24 in the bimodality table); also record the environment/cgroup/limits of a watcher-launched server vs a bad one (cat /proc/<pid>/status, /proc/<pid>/cgroup, ulimit) so the difference is on file. If both screens are bad, the 22:50 reporting rule stands unchanged (B@10000 as the reproducible lane-B row) and you stop here. Other row state at 23:06 (all / dead / live): A_R3_20000 81/127, 15 dead, 72% live; A_R2_20000 29/98, 39 dead (!), 49% live; A_R3_17500b 26/58, 12 dead, 57%; A_R3_15000b 2/48, 28 dead; B_R3_17500 10/89, 1 dead, 11%; B_R2_17500 43/101, 15 dead, 50%. Full text: 23:10 entry in plan/ORCHESTRATOR_NOTES.md.

## 22:50 UTC (2026-09-05) — the lane-B late-checkpoint collapse: everything excluded tonight, and how the report must treat it

STOP CHASING THE LANE-B COLLAPSE TONIGHT; HOW TO REPORT IT. Excluded since 22:05, all measured: the seed (B@17500 unseeded eval-28 1/18 and seed-12345 eval-29 1/18, identical failure profile: 12 at progress 0.5, 4 at 0.67); the checkpoint files (no file in any lane-B checkpoint dir touched after creation; statistics.json identical across checkpoints and the rescued copy; the 17500 weights sha256 match the rescue); the device (dev 2 hosted a 19/20 and a 1/20 run); the process environment (watcher vs ssh vs a bad row's server: no thread/numeric/library variable differs); the decoder provider (always CPUExecutionProvider); stale servers on ports (one server per running row, none left from stopped runs); code drift (only the opt-in --fresh-first-obs and --seed changes, and B@20000 screened 15/20 after both). Unexplained fact for the record: every good run of a late lane-B checkpoint was a WATCHER screen (eval-7 12500 16/20 at 17:14, eval-14 17500 19/20 at 19:10, eval-23 20000 15/20 at 20:22 — 3/3) and every other launch of 12500/17500/20000 was bad (eval-8, 10, 24, 26, 28, 29 — 6/6 so far, plus 15000's only run 4/20), failures at the right-arm phase. Two more unseeded B@17500 screens are running as a bimodality test (eval-30 port 8913, eval-31 port 8914; results ~23:05): read them, record them, do not launch more. REPORTING RULE: (1) lane B round 3's best REPRODUCIBLE row is checkpoint-10000: 185/200 unseeded (177/180 held-out; 7 dead starts) with screens 16/18/19/18/19/19/19 of 20 and 17/27 seeded — quote it as lane B's round-3 result; (2) checkpoints 12500/17500/20000 go into the report as an UNRESOLVED run-level bimodality with the complete table (run, launcher, time, load, seed, device, port, outcome, failure milestone) and the exclusion list above; the pre-registered pick (17500) keeps its row (eval-24, ending ~00:30) in the table but the verdict sentence must not rest on a single collapsed run; (3) lane A round 3 = A_R3_20000 (eval-9) headline plus eval-13/eval-14; (4) round-2 headline checkpoints re-measured = eval-10 (A) and eval-25 (B); (5) every row: per-episode string, dead count, all-200 and 20-199, verdict on live episodes of the 20-199 slice; (6) harness debts, in this order: dead starts at reset (load-dependent), the late-lane-B bimodality, the rejected fresh-first-obs patch. Then GATE P11 and close. Full text: 22:50 entry in plan/ORCHESTRATOR_NOTES.md.

## 21:45 UTC (2026-09-05) — load-drop experiment: four rows stopped, the four headline rows continue at 4-way load; read eval-24's string from episode ~21 on

LOAD-DROP EXPERIMENT ON THE ROWS (do not relaunch what I stopped). At 8-way load the lane-B round-3 headline rows collapsed in a way their quiet-time screens did not: B_R3_17500 (eval-24) 1/20 with 16 failures at progress 0.5 (right arm never reaches) against its 19/20 screen at 19:00; B_R3_20000 (eval-26) 1/20 against 15/20; B_R3_10000 (eval-27) 17/27 is fine; lane A rows are only losing dead starts (A_R3_20000 20/31 with 4 dead). Server latency p90 rose from ~157 ms (quiet) to ~370 ms with 4.7 s spikes, sim 45 -> 58 ms/step, load average 317 on 384 cores. Whether that collapse is the render-lag artefact in its mid-episode form or a real property of those checkpoints is the question that decides the report, so at 21:44 I SIGTERMed (recorded launcher pgids, mine, listed in /tmp/franka-sonic/p11/orch_rows.txt) the four least important rows — A_R3_15000 (eval-12, 13 dead of 21), A_R3_17500 (eval-11), B_R3_20000 (eval-26), B_R3_10000 (eval-27; the unseeded 185/200 row stands) — and kept the four that make the round-2 vs round-3 headline comparison: A_R3_20000 (eval-9), B_R3_17500 (eval-24), A_R2_20000 (eval-10), B_R2_17500 (eval-25), now at 4-way load. READ THE PER-EPISODE STRING of eval-24 from episode ~21 onward: if it recovers toward its screen rate, the collapse was load and the report must say every row's rate depends on concurrent load (and the four stopped rows re-run later in a second 4-way batch); if it stays near 0/20, checkpoint-17500 is genuinely fragile at 200 rollouts and the pre-registered lane-B headline is what it is. Their partial results stay on disk (nothing deleted). Report the stopped rows as "stopped at episode N by the orchestrator for the load experiment". Full text: 21:45 entry in plan/ORCHESTRATOR_NOTES.md.
